...
const isColorSupported = chalk.enabled; 
const banner = [
    isColorSupported ? chalk.bgCyan.black('============================================================') : '============================================================',
    isColorSupported ? chalk.bgGreen.black('=======================  J.W.P.A  ==========================') : '=======================  J.W.P.A  =========================',
    isColorSupported ? chalk.bgMagenta.white('================= @AirdropJP_JawaPride =====================') : '================= @AirdropJP_JawaPride =====================',
    isColorSupported ? chalk.bgYellow.black('=============== https://x.com/JAWAPRIDE_ID =================') : '=============== https://x.com/JAWAPRIDE_ID =================',
    isColorSupported ? chalk.bgRed.white('============= https://linktr.ee/Jawa_Pride_ID ==============') : '============= https://linktr.ee/Jawa_Pride_ID ==============',
    isColorSupported ? chalk.bgBlue.black('============================================================') : '============================================================',
];
        ...
        if (value) {
            switch(level) {
                case 'error':
                    ...
                    break;
                case 'warn':
                    formattedValue = chalk.bgYellow(`🔔 ${value}`);
                    break;
                case 'success':
...
async function askQuestion(question) {
    ...
    return new Promise((resolve) => {
        rl.question(chalk.bgGrey(`💬 ${question}`), (answer) => {
            rl.close();
            resolve(answer);
        });
    });
...
async function autoRegister() {
    console.log(banner);
    logger.info('AUTO REFF JAWA', 'J.W.P.A');
    ...
    const numberOfWallets = parseInt(await askQuestion("how many reff do you need create wallet? "));
    if (isNaN(numberOfWallets) || numberOfWallets <= 0) {
        logger.error('Invalid number of wallets specified');
        return;
    }

    const refCode = await askQuestion("Enter your referral code (example: T5zRD7dz): ");
    if (!refCode) {
